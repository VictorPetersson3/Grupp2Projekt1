...
public class SplineManager : MonoBehaviour
{
    ...
    public Vector2 GetClosestPoint(Vector2 aPosition, ref int aPointsIndex, ref Vector2[] aPoints)
    {
        ...
        {
            Vector2 closestPointInIteration = pathCreators[i].GetClosestPoint(aPosition, ref aPointsIndex);

            if (Vector2.Distance(aPosition, closestPointInIteration) < Vector2.Distance(aPosition, closestPoint))
            {
